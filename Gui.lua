...
local OceanFolder = "Ocean"
local ConfigurationFolder = OceanFolder.."/Configurations"
local ConfigurationExtension = ".ocean"
...
local Ocean = game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn.protect_gui then 
	syn.protect_gui(Ocean)
	Ocean.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Ocean.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Ocean.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Ocean.Name and Interface ~= Ocean then
			Interface.Enabled = false
			Interface.Name = "Ocean"
		end
	end
else
...
local Main = Ocean.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Ocean.DisplayOrder = 100
...
local Notifications = Ocean.Notifications

local SelectedTheme = OceanLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = OceanLibrary.Theme[ThemeName]
	...
	Ocean.Main.BackgroundColor3 = SelectedTheme.Background
	Ocean.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Ocean.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Ocean.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Ocean.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Ocean.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Ocean.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if OceanLibrary.Flags[FlagName] then
			spawn(function() 
				if OceanLibrary.Flags[FlagName].Type == "ColorPicker" then
					OceanLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if OceanLibrary.Flags[FlagName].CurrentValue or OceanLibrary.Flags[FlagName].CurrentKeybind or OceanLibrary.Flags[FlagName].CurrentOption or OceanLibrary.Flags[FlagName].Color ~= FlagValue then OceanLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
		end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(OceanLibrary.Flags) do
...
function OceanLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= OceanLibrary.Theme.Default then
...
function OceanLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Ocean Hub | "
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "By Yoolovr"
	if Settings.LoadingTitle ~= "Game Name" then
		LoadingFrame.Version.Text = "Ocean Hub"
	end
	...
	pcall(function()
		...
		if not isfolder(OceanFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(OceanFolder.."/Discord Invites") then
			makefolder(OceanFolder.."/Discord Invites")
		end
		if not isfile(OceanFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then 
				writefile(OceanFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Ocean RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(OceanFolder.."/Key System") then
			makefolder(OceanFolder.."/Key System")
		end
		...
		if isfile(OceanFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(OceanFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(OceanFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
					end
				else
		...
		function Tab:CreateColorPicker(ColorPickerSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					OceanLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					OceanLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					OceanLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					OceanLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					OceanLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function OceanLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
